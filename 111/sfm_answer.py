...
def get_reprojection_residuals(points2d: np.ndarray, points3d: np.ndarray, intrinsics: np.ndarray,
    ...
    # 将3D点齐次化，通过添加第四个坐标（1）将它们转换为齐次坐标
...
def solve_pnp(image_id: str, point2d_idxs: np.ndarray, all_points3d: np.ndarray, point3d_idxs: np.ndarray,
    ...
    for _ in range(num_ransac_iterations):
        ...
        """
        # 使用cv2.solvePnP计算相机姿态
        # selected_pts3d 是从3D空间中选取的点集，形状为(N, 3)
        # selected_pts2d 是这些点在图像上的对应点集，形状为(N, 2)
        # intrinsics 是相机的内参矩阵，形状为(3, 3)
        # distCoeffs 是相机的畸变系数，这里设为None表示没有畸变
        # flags 指定求解PnP问题的算法，这里使用cv2.SOLVEPNP_ITERATIVE进行迭代求解
        _, rotation_vector, tvec = cv2.solvePnP(objectPoints=selected_pts3d,
                                                imagePoints=selected_pts2d, 
                                                cameraMatrix=intrinsics, 
                                                distCoeffs=None, 
                                                flags=cv2.SOLVEPNP_ITERATIVE)
        # 使用cv2.Rodrigues将旋转向量转换为旋转矩阵
        # 旋转矩阵rotation_mtx的形状为(3, 3)
        rotation_mtx, _ = cv2.Rodrigues(rotation_vector)
        # 计算重投影残差
        # 定义一个函数get_reprojection_residuals来计算实际2D点与通过投影矩阵计算得到的2D点之间的距离
        # 这里假设该函数已经被正确实现，并且接受以下参数：
        # points2d: 实际的2D点集，形状为(N, 2)
        # points3d: 对应的3D点集，形状为(N, 3)
        # intrinsics: 相机的内参矩阵，形状为(3, 3)
        # rotation_mtx: 旋转矩阵，形状为(3, 3)
        # tvec: 平移向量，形状为(3, 1)或(3,)
        # 函数返回所有点的重投影残差数组
...
def add_points3d(image_id1: str, image_id2: str, all_extrinsic: dict, intrinsics, points3d: np.ndarray,
    ...
    """
    new_points3d = triangulate(image_id1=image_id1, # 第一个图像的标识符
                               image_id2=image_id2, # 第二个图像的标识符
                               kp_idxs1=matches[:, 0], # 第一个图像中匹配特征点的索引
                               kp_idxs2=matches[:, 1], # 第二个图像中匹配特征点的索引
                               extrinsics1=all_extrinsic[image_id1],# 第一个图像的外参矩阵
                               extrinsics2=all_extrinsic[image_id2],# 第二个图像的外参矩阵
                               intrinsics=intrinsics)# 相机的内参矩阵
    """ END YOUR CODE HERE """
...
def get_next_pair(scene_graph: dict, registered_ids: list):
    ...
    """ YOUR CODE HERE """
    for registered_id in registered_ids:# 遍历所有已注册的图像ID
        neighbors = scene_graph[registered_id] # 获取当前已注册图像的相邻图像ID列表（即邻居）
        for new_id in neighbors:# 遍历当前已注册图像的邻居
            if new_id not in registered_ids:# 检查邻居图像是否未被注册
                # 加载当前图像对之间的匹配信息和内点数量
                # 假设load_matches函数返回一个包含匹配点对和内点数量的元组
                matches, num_inliers = load_matches(registered_id, new_id)
                if num_inliers > max_num_inliers:# 如果当前图像对的内点数量大于当前记录的最大内点数量
                    max_num_inliers = num_inliers# 更新最大内点数量、未注册图像ID和已注册图像ID
                    max_new_id = new_id
