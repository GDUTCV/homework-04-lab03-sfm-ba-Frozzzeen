...
def detect_keypoints(image_file: os.path):
    ...
    image = cv2.imread(image_file)# 默认读取彩色图像，SIFT通常用于彩色图像
    if image is None:
        raise ValueError(f"无法加载图像文件: {image_file}")
    # 创建SIFT特征检测器对象并检测关键点
...
def create_feature_matches(image_file1: os.path, image_file2: os.path, lowe_ratio: float = 0.6, min_matches: int = 10):
    ...
    """
    bf = cv2.BFMatcher()# 使用BFMatcher进行KNN匹配，这里使用默认的L2距离（欧氏距离）
    matches = bf.knnMatch(descriptors1, descriptors2, 2)# 进行KNN匹配，k=2表示我们想要每个特征点的两个最佳匹配

    # Apply ratio test应用比率测试
    for m, n in matches:
        if m.distance < lowe_ratio * n.distance:
            # 注意：这里我们只需要m，因为n是次优匹配，仅用于比率测试
...
def create_ransac_matches(image_file1: os.path, image_file2: os.path,
    ...
    if not os.path.exists(feature_match_file):
        return match_id  # images are not matched under feature matching or the iaswswsmage_id1 >= image_id2
    ...
    """
    # 计算基础矩阵
    essential_mtx, is_inlier= cv2.findEssentialMat(points1=points1,
                                                   points2=points2,
...
def create_scene_graph(image_files: list, min_num_inliers: int = 40):
    ...
    for i in range(len(image_ids)):
        id_1 = image_ids[i]# 获取当前外层循环的图像ID
        for j in range(i+1, len(image_ids)):
            id_2 = image_ids[j]# 获取当前内层循环的图像ID
            # 构造匹配文件的ID，格式为"image1_image2"
            ...
            if os.path.exists(match_save_file):
                # 加载内点数（这里假设.npy文件存储的是内点的索引或布尔数组，我们需要计算其长度或True值的数量）
                # 注意：如果.npy文件存储的是内点的布尔掩码，则使用np.sum(inliers)来获取内点数
                # 如果存储的是内点索引，则使用len(inliers)可能不正确，除非它是经过筛选的索引列表
                # 这里我们假设它存储的是内点的索引列表，并且我们需要检查其长度
                inliers = np.load(match_save_file)
                # 检查内点数是否足够（即大于或等于指定的最小内点数阈值）
                if len(inliers) > min_num_inliers:# 检查内点数是否足够， 如果内点数足够，在图中添加一条边，表示这两个图像之间有足够的匹配点
                    graph.add_edge(i, j)# 在图中添加边， i和j是图像在image_files列表中的索引，也是图中的节点
